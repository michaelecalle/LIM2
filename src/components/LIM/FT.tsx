...
function findRowIndexFromPk(targetPk: number | null): number | null {
  ...
  useEffect(() => {
    ...
    const handler = (e: Event) => {
      ...
      // Réseau (ADIF/LFP/RFN) déduit de s_km — MÊME base que le moteur PK et que la
      // bascule du numéro (fiable, monotone). Diffusé au bloc info (catégorie), au
      // changement seulement. Pas de s_km valide (tunnel/GPS perdu) → pas d'émission,
      // donc la catégorie reste gelée jusqu'au retour du GPS (comportement voulu).
      const netSkm = networkFromSkm((detail as any).s_km);
      if (netSkm && lastEmittedNetworkRef.current !== netSkm) {
        lastEmittedNetworkRef.current = netSkm;
        window.dispatchEvent(
          new CustomEvent("lim:network-change", { detail: { network: netSkm } })
        );
      }
