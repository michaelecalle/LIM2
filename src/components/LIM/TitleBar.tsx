...
type MixedTrainIdentificationMethod =
  ...
  const MIXED_TRAIN_OCR_TIMEOUT_MS = 60_000
...
function normalizeKnownTrainNumber(value?: string | null): string | null {
...
function buildDetectedTrainTokenVariants(value: string): string[] {
  const digits = String(value ?? '').replace(/\D/g, '')
  if (!digits) return []
  ...
  // On enlève le 0 initial et le 0 inséré après le 9.
...
function detectedTokenMatchesKnownTrain(
  ...
  return buildDetectedTrainTokenVariants(detectedToken).some((variant) => {
    return normalizeKnownTrainNumber(variant) === normalizedKnown
  })
}

function findUniqueTrainInText(
  text: string,
  ...
  for (const token of tokens) {
    for (const train of trainOptions) {
      ...
      if (match) {
        matches.set(train.trainNumber, train)
      }
    }
  }
...
function ManualPdfCanvasViewer({
  pdfUrl,
  page,
  onPageChange,
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        console.warn('[TitleBar] Manuel PDF canvas: chargement impossible', err)
    ...
    ;(async () => {
      ...
      try {
        ...
        const scale = Math.max(
          0.25,
          Math.min(3, availableWidth / baseViewport.width)
        )
        ...
        console.warn('[TitleBar] Manuel PDF canvas: rendu impossible', err)
