...
async function extractAllPagesTextAndDoc(file: File): Promise<{
  ...
  const diag = (message: string) => {
    window.dispatchEvent(
      new CustomEvent("ltv:diag", {
        detail: { message },
      })
    )
  }

  diag("[ltvParser] extract step 1 : avant file.arrayBuffer")

  const arrayBuffer = await file.arrayBuffer()

  diag("[ltvParser] extract step 2 : après file.arrayBuffer")

  const data = new Uint8Array(arrayBuffer)

  diag("[ltvParser] extract step 3 : avant getDocument")
  ...
  diag("[ltvParser] extract step 4 : après getDocument, avant promise")

  const pdf: PDFDocumentProxy = await loadingTask.promise

  diag("[ltvParser] extract step 5 : pdf chargé pages=" + pdf.numPages)

  const pagesText: string[] = []

  for (let i = 1; i <= pdf.numPages; i++) {
    diag("[ltvParser] extract page " + i + " : avant getPage")
    const page = await pdf.getPage(i)

    diag("[ltvParser] extract page " + i + " : avant getTextContent")
    ...
    try {
      ...
      diag(
        "[ltvParser] getTextContent KO page " +
          i +
          " : on continue sans texte"
      )
      pageText = ""
    }
  ...
  diag("[ltvParser] extract terminé pagesText=" + pagesText.length)
...
async function handleFileForLtv(file: File) {
  ...
  diag("[ltvParser] handleFileForLtv START : " + file.name)

  let pdf: PDFDocumentProxy
  let pagesText: string[]

  try {
    diag("[ltvParser] avant extractAllPagesTextAndDoc")
    const extracted = await extractAllPagesTextAndDoc(file)
    pdf = extracted.pdf
    pagesText = extracted.pagesText
    diag("[ltvParser] après extractAllPagesTextAndDoc | pages=" + pagesText.length)
  } catch (err) {
    diag("[ltvParser] ERREUR extractAllPagesTextAndDoc : " + String(err))
    return {
      mode: "NO_LTV",
    } as LTVParseResult
  }
  ...
  diag("[ltvParser] classifyLtvDisplayInternal : " + c.mode)
...
function setup() {
  ...
  const onImport = (e: Event) => {
    ...
    ltvDiag(
      "[ltvParser] lim:import-pdf reçu | fichier = " +
        (file ? `${file.name} (${file.size} octets)` : "AUCUN")
    )

    if (file) {
      ltvDiag("[ltvParser] appel handleFileForLtv")
